...
def create_makefile_idl():
    ...
    f.write(f"\t@idl -e \"efs\"\n")
    f.write(f"\t@python {cwd}/read_csv_python.py\n")
    ...
    f.write(f"\t@mv {datadir}/data/w_s {snrdir}/data_files/.\n")
...
def create_makefile_noidl():
    ...
    f.write("isnt echoed in the terminal\n")
    f.write("setup:\n")
    f.write(f"\t@tar -xf {dldir}/efs_Jesper.tar.gz\n")
    f.write(f"\t@rm -rf {snrdir}/data_files/w_s\n")
    f.write(f"\t@python {cwd}/mince_eig.py\n")
    f.write(f"\t@mv {dldir}/w_samarth.dat {datadir}/data/w_s/.\n")
    # f.write("# making necessary directory structure\n")
    # f.write("\t# cloning the GitHub repo to get eig_files and data_files to store in snrnmais\n")
    ...
    f.write(f"\t@rm -rf {datadir}/snrnmais_files\n")
    f.write(f"\t@rm -rf {dldir}\n")
    f.write(f"\t@rm -rf {datadir}/data/w_s")
    # f.write(f"\t@rm -rf *.egg-info\n")
    return None
...
def download_files(has_idl):
    if(has_idl):
        dlfiles_fname = f'{cwd}/.dl_files_idl'
    else:
        dlfiles_fname = f'{cwd}/.dl_files_noidl'

    with open(dlfiles_fname, 'r') as f:
...
if __name__ == "__main__":
    ...
    while(True):
        idl_y_or_n = input("Do you have IDL on your system? (y/n)")
        if(idl_y_or_n == 'y' or idl_y_or_n == 'Y'): 
            has_idl = True
            break
        elif(idl_y_or_n == 'n' or idl_y_or_n == 'N'): 
            has_idl = False
            break
        else:
    ...
    download_files(has_idl)
    if(has_idl): create_makefile_idl()
    else: create_makefile_noidl()
    os.system("make")
